...
const Lobby = ({ onSelect }) => {
  ...
  useEffect(() => {
    ...
    const usersRef = ref(db, 'app/users')
    ...
    <View style={lobbyStyle}>
      <ScrollView>
        {users && users.map((user) => (
          <Quester key={user.id} name={user.Name} number={user.Phone}
            onCheck={() => {
              // const obj = {}
              // obj[user.id] = {
              //   name: user.Name,
  ...
  return (
    <View style={questerStyle}>
      ...
      <View style={questerStyle}>
        ...
        <CustomCheckbox handleCheck={() => { setIsSelected(true) }} handleUncheck={() => { setIsSelected(false) }} />
      </View>
    </View>
  );
};
